...
class EyeDataGrid
{
	...
	const STDCTRL_DEACTIVATE = 5;
	const STDCTRL_ACTIVATE = 6;
	const TYPE_DATE = 1;
	const TYPE_IMAGE = 2;
	...
	public function addStandardControl($type, $action, $action_type = self::TYPE_ONCLICK)
	{
		...
		switch ($type)
		{
			case self::STDCTRL_EDIT:
				...
				break;
			case self::STDCTRL_DEACTIVATE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="Deactivate" title="Deactivate" class="tbl-control-image"></a>';
				break;
			case self::STDCTRL_ACTIVATE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_create . '" alt="Activate" title="Activate" class="tbl-control-image"></a>';
				break;
			default:
	...
	*/
	private function buildControls(array $row)
	{
			...
			if (count($this->controls) > 0)
			{
				...
				echo $this->parseVariables($row, $this->controls[0]);
				if($row['Status'] == 'Not Active'){
						echo $this->parseVariables($row, $this->controls[1]);
				}else if($row['Status'] == 'Active'){
						echo $this->parseVariables($row, $this->controls[2]);
				}
