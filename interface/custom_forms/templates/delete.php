...
if ($_POST['confirm']) {
	...
	$strSql =	"select form_id from forms where id =".$form_id;
	$stmt	=	sqlStatement($strSql);
	$row	=	sqlFetchArray($stmt);
	
	$strSql = "DELETE from csf_form_data where form_id =".$row['form_id'] ." and encounter = ".$_SESSION['encounter'];
	sqlQuery( $strSql );
