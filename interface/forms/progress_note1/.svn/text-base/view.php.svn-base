...
// include_once("../phyauraforms_common/common_functions.php");
...
$sql .= " ${MAIN_TABLE_NAME}_cb_sel a";
$sql .= ",${MAIN_TABLE_NAME}_cb_flds b";
$sql .= " where a.fk_${MAIN_TABLE_NAME}_header=$form_id and a.fk_${MAIN_TABLE_NAME}_cb_flds = b.id";
$stmt=sqlStatement($sql);
while($row=sqlFetchArray($stmt)) {
	$key = "cb_".$row['name'];
